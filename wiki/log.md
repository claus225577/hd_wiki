...
## 2026-06-25 — LinkedIn-research ingest (GSU-DBNet + SE-AGCNet)

**Operation:** Daily LinkedIn-pair research ingest. Two June-2026 arXiv preprints in the speech-enhancement / hearing-aid axis were drafted to LinkedIn (posted to Discord channel 1492462147127742565, emailed to door.punch_4o@icloud.com, archived to ~/linkedin-drafts repo and pushed to github.com/claus225577/linkedin-drafts). Both papers reshape parts of the HA processing chain that have been stable for 10-25 years.

### Source files — net-new (2)

- **`sources/arxiv-2606-23761-gsu-dbnet-neuromorphic-se-jun-2026.md`** — Taiyu Meng, Wenbin Jiang, Haoyi Zhang, Yuhan Zhou, Haibing Yin — arXiv:2606.23761, 22 Jun 2026 — **GSU-DBNet**. Dual-branch SNN modelling magnitude and complex spectra simultaneously; new primitive **Gated Spiking Unit (GSU)** (learned gate over spike output, preserves event-driven sparsity) used along temporal and frequency axes. **394K parameters, PESQ 3.04**, **4.5–10.6% of representative ANN-baseline param counts** at competitive quality — first arXiv pure-SNN SE result inside a realistic hearing-aid parameter budget. Open questions called out: neuromorphic-substrate maturity (Loihi 2 / SpiNNaker 2 exist but no HA OEM ships neuromorphic silicon today), algorithmic latency (not in abstract; HA needs <10 ms), concrete µJ/inference numbers.

- **`sources/arxiv-2606-25959-se-agcnet-joint-se-agc-jun-2026.md`** — Jinming Zhang, Wei Rao, Xionghu Zhong, Eng Siong Chng — arXiv:2606.25959, 24 Jun 2026, **accepted Interspeech 2026** — **SE-AGCNet**. End-to-end joint optimisation of speech enhancement + automatic gain control with a loudness-aware loss (integrated LUFS, short-term LUFS, LRA) + paired synthesis pipeline **SE-AGC-DataGen**. Formally documents two cascade failure modes: AGC→SE amplifies background noise into the SE input; SE→AGC over-suppresses low-volume speech that the gain stage was supposed to recover (the canonical fitting-room "it's quieter, but the soft talkers got quieter too" complaint). Open questions called out for HA deployment: latency (meeting scenarios tolerate hundreds of ms, HA budget <10 ms), fitting interface (what replaces NAL-NL2 / DSL v5 prescription targets when WDRC is a learned block), per-ear audiogram conditioning, attack/release time-constant exposure.

### Wiki pages created (2)

1. **`wiki/concepts/spiking-neural-networks-speech-enhancement.md`** — SNN SE as a hearing-AI concept page. Frames the two architectural axes: **pure-SNN** (GSU-DBNet, future neuromorphic substrate) vs **hybrid SNN+ANN** (DBHN-Net, standard hardware today). Documents the two limitations the June-2026 work addresses (binary-activation coarseness; architecture lag behind ANN SE) and the recipes for narrowing them (gating, dual-branch magnitude+complex, two-axis exploitation). Open questions: latency on shipped silicon, concrete energy numbers on neuromorphic substrates, per-wearer personalisation for SNN SE. Implication: the 2028 HA SoC architectural review should explicitly contain a "what if SNN is real?" branch even if the conclusion is "stay on ANN." Cross-links: parent `speech-enhancement-neural-networks`, `dnn-architectures-hearing-aids`, `dnn-in-hearing-aids`, `on-device-ml-hearing-aids`, `small-language-models-edge-ai`, `model-compression`, `tinyml-edge-ai`, `compute-in-memory`.

2. **`wiki/concepts/joint-se-wdrc-end-to-end.md`** — Joint SE + WDRC / AGC as a hearing-AI concept page. Names the 25-year NR→WDRC cascade as the artefact it is — downstream of when SE was DSP, WDRC was DSP, and the two teams didn't share a loss function. Documents the SE-AGCNet failure-mode table and the LUFS-aware joint-training recipe. Argues PESQ/STOI/SI-SDR are intelligibility *surrogates* and LUFS-aware losses align the optimisation target with what audiologists actually fit against (NAL-NL2 / DSL v5 are loudness-balanced prescriptions, not spectrogram-distance prescriptions). Reframes the Korhonen / Widex "DNN-vs-natural-processing" debate as plausibly an artefact of the same separate-loss problem; joint training could close the gap. Implications for 2028: one learned NR+WDRC block trained end-to-end, audiogram conditioning, explicit fitter-exposed levers; the fitting-protocol design problem is harder than the architecture. Cross-links: `speech-enhancement-neural-networks`, `dnn-in-hearing-aids`, `dnn-architectures-hearing-aids`, `differentiable-dsp`, `nal-nl2-fitting`, `listening-effort-evaluation`, `dnn-vs-natural-processing`, `on-device-ml-hearing-aids`.

### Wiki pages updated (1)

1. **`wiki/concepts/speech-enhancement-neural-networks.md`** — Frontmatter: added both new sources, added both new concept pages to `related:`, new tags `spiking-neural-network / snn / neuromorphic / gated-spiking-unit / gsu / joint-se-agc / wdrc / lufs / end-to-end-loudness`; bumped `updated:` to 2026-06-25 with `last_change` note documenting both June-2026 entries (GSU-DBNet pure-SNN parameter-budget alignment + SE-AGCNet cascade artefact + joint training).

### Editorial notes

- **Did NOT create a dedicated entity page for the GSU-DBNet authors (Hangzhou Dianzi University audio group, Yin et al.)** — single-paper author cluster doesn't yet justify an entity page per the wiki orphan policy. Re-evaluate at second author-group HA-relevant arXiv drop.
- **Did NOT create a dedicated entity page for the SE-AGCNet authors (Tencent Ethereal Audio Lab + Hunan University + NTU Singapore)** — same reason. Eng Siong Chng's group at NTU has multiple SE papers in 2026 but they sit across enough subtopics that a single entity page would dilute. Re-evaluate.
- **Did NOT update `concepts/dnn-in-hearing-aids.md` or `concepts/on-device-ml-hearing-aids.md`** with the GSU-DBNet datapoint. SNN SE is a structurally distinct architectural axis (event-driven vs MAC-array) that warrants its own concept page; folding it into the broader DNN-in-HA page would either bury it or distort the page's scope. Re-evaluate at horizontal lint pass.
- **Did NOT update `syntheses/on-chip-hearing-ai-levers-june-2026.md`** with SNNs as a sixth lever. The lever list in that synthesis is currently five hardware-agnostic compression/efficiency moves on conventional silicon (memory bandwidth, sparsity, inference steps, frame rate, algebraic weight sharing); SNNs would extend the synthesis to mixed conventional/neuromorphic substrates and that broader axis-shift is worth one careful pass rather than a casual append. Re-evaluate.
- **Did NOT update `entities/sonova-ag.md` / `entities/demant-oticon.md` / `entities/starkey.md`** — no OEM has publicly committed to SNN SE silicon or joint-SE-WDRC training pipelines yet; ingesting research preprints into OEM entity pages would over-attribute.
